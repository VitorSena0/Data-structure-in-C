...
void checksum(char *string, uint32_t *somaBitaBit, uint16_t *i) {
    while (*string != '\0') {
        *somaBitaBit ^= (uint32_t)((*string));// poderia por o *i multiplicando o valor de *string para considerar o caso de uma string ter as memas letras mas em ordem diferente, assim a ordem inportaria e consequentemente a probabilidade de colisão seria menor
        string++;
        (*i)++;
    }
}
