...
void processarArquivo(const char *input, const char *output) {
    ...
    if(fscanf(arquivoInput, "%d", &qtdTermos) != 1){
        printf("Erro ao ler a quantidade de termos\n");
        fclose(arquivoInput);
        fclose(arquivoInput);
        return;
    }
    ...
    for(int i = 0; i < qtdTermos; i++) {
        if(fscanf(arquivoInput, "%21s", palavra) != 1){
            printf("Erro ao ler a palavra %d\n", i);
            fclose(arquivoInput);
            fclose(arquivoOut);
            return;
        }
    ...
    if(fscanf(arquivoInput, "%d", &qtdRequisicoes) != 1){
        printf("Erro ao ler a quantidade de requisições\n");
        fclose(arquivoInput);
        fclose(arquivoOut);
        return;
    }
